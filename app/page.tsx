...
<div className="w-full relative">
  ...
  {/* القائمة المنسدلة المحدثة (داكنة وواضحة) */}
  {isOpenCountry && (
    <div className="absolute top-full left-0 right-0 mt-2 bg-black border border-[#D4AF37] rounded-xl shadow-2xl max-h-60 overflow-y-auto p-2 z-[999]">
      <div className="relative mb-2 sticky top-0 bg-black">
        <input 
          autoFocus 
          placeholder="ابحث..." 
          className="w-full p-3 border-b border-[#D4AF37] outline-none bg-transparent text-white font-bold" 
          value={searchQuery} 
      ...
      {filteredCountries.map(c => (
        <div 
          key={c} 
          className="p-4 hover:bg-[#D4AF37] border-b border-white/10 text-right cursor-pointer text-white font-bold transition-colors" 
          onClick={() => { setCountry(c); setIsOpenCountry(false); setSearchQuery(""); }}
        >
          {c}
        </div>
      ))}
